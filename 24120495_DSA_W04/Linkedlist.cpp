...
void removeTail(List*& L)
{
    ...
    while (temp->p_next != nullptr && temp->p_next != L->p_tail)
    {
        temp = temp->p_next;
    }
...
void removeAfter(List*& L, int val)
{
    if (L==nullptr || L->p_head == nullptr)
        return;
    NODE* temp = L->p_head;
    while (temp != nullptr && temp->p_next != nullptr)
    {
        if (temp->key == val)
        {
...
{
    int n=countElements(L);
    if (L == nullptr || L->p_head == nullptr || pos >= n || pos < 0)
        return;
    if (pos == 0)
    {
        removeHead(L);
    ...
    NODE* inpos = temp->p_next;
    temp->p_next = inpos->p_next;
    delete inpos;
}
...
List* reverseList(List* L)
{
    ...
    List* reversed = createList(nullptr);
    NODE* temp = L->p_head;

    while (temp != nullptr)
    {
        addHead(reversed, temp->key);
        temp = temp->p_next;
    }
