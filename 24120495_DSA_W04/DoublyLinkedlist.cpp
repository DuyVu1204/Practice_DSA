...
struct List
{
    NODE* pHead;
    NODE* pTail;
    List(NODE* head = nullptr, NODE* tail = nullptr) : pHead(head), pTail(tail) {}
};
...
{
    NODE* current1 = list1 ? list1->pHead : nullptr;
    NODE* current2 = list2 ? list2->pHead : nullptr;
...
void removeAfter(List*& L, int val)
{
    if (L->pHead == nullptr || L->pHead==nullptr)
        return;
    NODE* temp = L->pHead;
    while (temp != nullptr && temp->pNext != nullptr)
    {
        if (temp->key == val)
        {
            NODE* after = temp->pNext;
            if (after == L->pTail)
            {
                L->pTail = temp;
            }
            else if (temp->pNext!=nullptr)
                after->pNext->pPrev = temp;
            temp->pNext = after->pNext;
            delete after;
            return;
        }
        temp = temp->pNext;
...
{
    int n = countElements(L);
    if (L == nullptr || L->pHead == nullptr || pos < 0 || pos >= n)
        return;
    if (pos == 0)
    {
        removeHead(L);
    ...
    NODE* inpos = temp->pNext;
    temp->pNext = inpos->pNext;
    inpos->pNext->pPrev = temp;
    delete inpos;
}
...
bool addBefore(List*& L, int data, int val)
{
    if (L == nullptr || L->pHead == nullptr)
        return false;
    if (L->pHead != nullptr && L->pHead->key == val)
    {
        addHead(L, data);
        return true;
    }
...
bool removeElement(List*& L, int key)
{
    if (L->pHead == nullptr)
        return false;
    while (L->pHead != nullptr && L->pHead->key == key)
        removeHead(L);
    if (L->pHead == nullptr)
        return true;
    NODE* cur = L->pHead;
